import { DocumentList, NotesAndFolder, Folder, Note, PromiseResponse, Resp_Note, Resp_Folder } from "../types";
...
export async function createRootFolder(folder_data: Folder): Promise<PromiseResponse<Resp_Folder>> {
  try {
    ...
    const existingFolders = await getFoldersByParentId(folder_data.parent_id, folder_data.user_id);
    
    if(existingFolders.error) return {error: true, message: 'Error creating folder, try again in a bit'}

    if (existingFolders.data.some(folder => folder.title === folder_data.title)) {
      return {error: true, message: `Folder with the name "${folder_data.title}" already exists in this folder.`};
    }
...
export async function createNote(file_data: Note): Promise<PromiseResponse<Resp_Note>> {
  try {

    const existingNotes = await getNotesByParentId(file_data.parent_id, file_data.user_id);
    
    if(existingNotes.error) return {error: true, message: 'Error creating file, try again in a bit'}

    if (existingNotes.data.some(folder => folder.title === file_data.title)) {
      return {error: true, message: `Note with the name "${file_data.title}" already exists in this folder.`};
    }
...
export async function createFolder(user_id: string, parent_id: string, title: string, parent_title: string): Promise<PromiseResponse<Resp_Folder>> {
  try {
    ...
    if (existingFolders.data.some(folder => folder.title === title)) {
      return {error: true, message: `Folder with the name "${title}" already exists in this folder.`};
    }
...
export async function getFoldersByParentId(parent_id: string | null, user_id: string): Promise<PromiseResponse<Resp_Folder[]>> {
  try {
    let queries: string[] = []

    if(parent_id) {
      queries = [Query.equal('user_id', [user_id]), Query.equal('parent_id', [parent_id])]
    } else {
      queries = [Query.equal('user_id', [user_id]), Query.equal('is_root', [true])]
    }
...
export async function getNotesByParentId(parent_id: string | null, user_id: string): Promise<PromiseResponse<Resp_Note[]>> {
  try {
    let queries: string[] = []
...
export async function getAllNotesAndFoldersFromParentID(parent_id: string, user_id: string): Promise<PromiseResponse<NotesAndFolder>> {
  try {
...
export async function getNoteById(note_id: string, user_id: string): Promise<PromiseResponse<Resp_Note>> {
  try {
