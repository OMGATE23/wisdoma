...
import { checkForRootFolder, getAllNotesAndFoldersFromParentID, getFolderById } from "../../utils/db"
    ...
    if(!userLoading && user) {
      async function fetchRootFolder() {
        try {
            ...
            const filesAndFolders = await getAllNotesAndFoldersFromParentID(parent_id, user!.id)
  ...
  return (
    <div className="h-[100vh] p-4 bg-gradient-to-bl from-blue-50 to-white">
      ...
      <div className="min-h-[90vh] flex items-center justify-center gap-8 w-[90%] mx-auto">
        ...
        {rootFolder && <RootFolder collection={filesAndFolder} rootFolder={rootFolder} loading={loading} />}
      </div>
    </div>
  )
}
