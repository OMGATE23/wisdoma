...
import { commonErrorHandling, getCurrentDateISOString } from '../../utils/helpers'
...
export default function RootFolder(props: Props) {
  ...
  async function createNoteInRoot(title: string): Promise<PromiseResponse<null>> {
    try {
  ...
  async function createFolderInRoot(title: string): Promise<PromiseResponse<null>> {
    try {
